 ...
 **/ 
 BOOLEAN add_to_scoreboard(scoreboard board, const score * sc) 
 {
    ...
    for(i = 0; i < SCOREBOARDSIZE; i++)
    {
        ...
        /* find a place in score table */
        if(sc->counters > board[i].counters)
        {
        	for(j = SCOREBOARDSIZE - 1; j > i; j--)
        	{
        		/* 
        		   replace the score to the next if place is found
				   by replace one by one start from the bottom and 
				   the one at the end of the table will be removed
        		 */
        		board[j].counters = board[j-1].counters;
